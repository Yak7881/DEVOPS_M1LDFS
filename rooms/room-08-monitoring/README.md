...
|-------|-------------------|
| **Monitoring** | Surveiller l'état de ton application (CPU, mémoire, etc.) en continu |
| **Logs** | Historique des événements générés par l'application (erreurs, requêtes, etc.) |
| **Métriques** | Données mesurables (ex : temps de réponse, nombre de requêtes) |
| **Dashboard** | Tableau de bord qui affiche les métriques en temps réel |
| **Alerts** | Notifications automatiques quand un seuil est dépassé (ex : CPU > 90 %) |
| **Uptime** | Pourcentage de temps où le service est disponible (ex : 99,9 %) |
...
- **Grafana** : crée des dashboards à partir des métriques
